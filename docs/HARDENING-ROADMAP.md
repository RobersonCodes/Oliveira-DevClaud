...
gestão/revogação de sessões também passou no CI Linux e smoke limpo. A Fase 6 foi concluída: auditoria
e ownership cross-tenant em secrets passaram no CI Linux/PostgreSQL e smoke limpo; as Fases 7-9 seguem pendentes —
ver Seção 7.
Uma rodada extraordinária de auditoria `main…HEAD` está em andamento em 2026-08-13 para eliminar o
oráculo de lockout no login, tornar o lint efetivo e restringir as permissões do CI. P1-14 foi
...
| P1-19 | 🟡 **Em correção (auditoria `main…HEAD`, 2026-08-13).** O login retornava `423 ACCOUNT_LOCKED` apenas para contas reais bloqueadas, enquanto e-mails inexistentes recebiam `401 INVALID_CREDENTIALS`, formando um oráculo de enumeração. A implementação agora sempre executa bcrypt (hash real ou sentinela) e responde `401 INVALID_CREDENTIALS` para conta existente com senha errada, inexistente ou bloqueada; o lockout continua persistido internamente. Regressão PostgreSQL real compara status, headers e corpo dos três casos. Validação local sem PostgreSQL: lint real, prova negativa do gate, typecheck e 48 testes sem infraestrutura passaram; conclusão aguarda CI Linux/PostgreSQL. | `apps/api/src/routes/auth.ts`; `apps/api/src/integration.test.ts`; `.github/workflows/ci.yml` |
...
avançar a Fase 7 por retries/backoff de jobs, seguida das demais etapas 7-9.
