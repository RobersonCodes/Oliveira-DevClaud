...
| P1-16 | 🟡 **Parcialmente corrigido (Fase 8, 2026-08-15; commits `2637473`, `3925c51` e `e6a0645`).** Manifest/ícone/tema, worker network-only, reconexão, bottom bar com 15 destinos e layout 320px/safe areas estão implementados. A auditoria encontrou 33 controles insuficientes; após a política mobile, Chromium mediu 118 controles em 16 rotas com zero abaixo de 44×44px. CIs `31860098544`/`31860101934` aprovaram 331 testes + 2 ignorados, build e audit zero; smoke `31860101978` verde. Ainda faltam toolbar/fluxos touch, retomada automática e testes físicos de notch/teclado/rotação/conectividade. | `apps/web/app/{layout.tsx,manifest.ts,mobile-navigation.tsx,pwa-lifecycle.tsx,icon.png,styles.css}`; `apps/web/public/sw.js`; `apps/web/src/{pwa.test.ts,mobile-navigation.test.ts}`; `apps/web/next.config.js` |
