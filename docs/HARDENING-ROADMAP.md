...
| P1-16 | 🟡 **Parcialmente corrigido (Fase 8, 2026-08-14).** Manifest App Router instalável, papel PWA/Apple para o ícone 1254×1254, tema/viewport com `viewport-fit=cover`, service worker registrado em produção e shell de reconexão foram implementados. O worker é deliberadamente network-only: não possui handler `fetch` nem Cache Storage, e `/sw.js` usa `no-store`, scope raiz, CSP própria e `nosniff`. Testes direcionados 3/3, typecheck, lint, build Next 16 e prova HTTP real passaram. Ainda faltam navegação mobile substituta, aplicação consistente de safe areas/layout a 320px, touch targets e testes físicos. | `apps/web/app/{layout.tsx,manifest.ts,pwa-lifecycle.tsx,icon.png,styles.css}`; `apps/web/public/sw.js`; `apps/web/src/pwa.test.ts`; `apps/web/next.config.js` |
