...
| P1-16 | 🟡 **Parcialmente corrigido (Fase 8, 2026-08-14; commits `2637473` e `3925c51`).** Manifest App Router instalável, ícone PWA/Apple, tema/viewport, worker network-only e shell de reconexão estão implementados. O corte mobile adicionou bottom bar persistente com todos os 15 destinos, dialog nativo para ferramentas, estado ativo e exclusão do login; safe areas nos quatro lados, espaço da home indicator e layout sem overflow a 320px. Testes PWA/mobile 6/6 e Chromium 15/15 rotas em `320/320` passaram; CIs `31839461813`/`31839464758` aprovaram 330 testes + 2 ignorados, build e audit zero, e smoke `31839464854` passou. Ainda faltam auditoria dos demais touch targets, toolbar/fluxos touch, retomada automática e testes físicos de notch/teclado/rotação/conectividade. | `apps/web/app/{layout.tsx,manifest.ts,mobile-navigation.tsx,pwa-lifecycle.tsx,icon.png,styles.css}`; `apps/web/public/sw.js`; `apps/web/src/{pwa.test.ts,mobile-navigation.test.ts}`; `apps/web/next.config.js` |
