...
gestão/revogação de sessões também passou no CI Linux e smoke limpo. A auditoria e uma correção de
ownership cross-tenant em secrets estão implementadas localmente e aguardam CI; as Fases 7-9 seguem pendentes —
ver Seção 7.
**Data:** 2026-08-12
...
| F6-AUDIT | ⏳ **Implementado localmente (Fase 6, 2026-08-12), aguardando CI.** Ações mutáveis já tinham cobertura ampla; foram fechadas lacunas em logout, início manual de orquestração, cancelamento de setup ainda em fila, emissão de runtime ticket e reconciliação de status terminal do agente. Metadata não contém credenciais, prompts ou logs. Regressão direcionada 29/29; typecheck/lint/build API verdes. | `apps/api/src/{audit-coverage.test.ts,integration.test.ts,runtimeGateway.test.ts,routes/auth.ts,routes/orchestrations.ts,routes/setup.ts,routes/agents.ts,lib/runtimeGateway.ts}` |
| F6-TENANT | ⏳ **Corrigido localmente (Fase 6, 2026-08-12), aguardando CI/PostgreSQL.** `POST /secrets` validava o papel na organização informada, mas não confirmava que `projectId`/`workspaceId` pertenciam a ela. A rota agora rejeita combinações ambíguas, valida a cadeia de ownership antes da escrita e usa `404` indistinguível para recurso ausente/cross-tenant. Regressão de formato 9/9; conjunto direcionado 38/38, typecheck/lint/build API verdes; integração real adicionada. | `apps/api/src/{routes/secrets.ts,routes/secrets.test.ts,integration.test.ts}` |
...
P1-1/P1-2/P1-3, o rate limit por identidade e P1-4/RBAC foram fechados.
...
avançar a Fase 6 pela validação CI da auditoria e da correção cross-tenant final, seguida das demais
etapas 6-9.
