...
| P1-16 | 🟡 **Parcialmente corrigido (Fase 8, 2026-08-14; commit `2637473`).** Manifest App Router instalável, papel PWA/Apple para o ícone 1254×1254, tema/viewport com `viewport-fit=cover`, service worker registrado em produção e shell de reconexão foram implementados. O worker é deliberadamente network-only: não possui handler `fetch` nem Cache Storage, e `/sw.js` usa `no-store`, scope raiz, CSP própria e `nosniff`. Prova HTTP real e testes direcionados 3/3 passaram; CIs `31830093847`/`31830096715` aprovaram a suíte completa (327 aprovados, 2 ignorados), build e audit, e o smoke `31830096679` ficou verde. Ainda faltam navegação mobile substituta, aplicação consistente de safe areas/layout a 320px, touch targets e testes físicos. | `apps/web/app/{layout.tsx,manifest.ts,pwa-lifecycle.tsx,icon.png,styles.css}`; `apps/web/public/sw.js`; `apps/web/src/pwa.test.ts`; `apps/web/next.config.js` |
