...
| P1-11 | 🟡 **Implementado localmente; validação Docker/CI pendente (Fase 7, 2026-08-14).** `destroy()` agora remove o bind mount somente depois do container/rede. O reaper periódico usa PostgreSQL como fonte de verdade, carência temporal, labels exatos e rechecagem por candidato; falha de banco/broker, symlink, recurso recente ou rede com endpoint desconhecido preserva o recurso para retry. A cota de disco já foi fechada em F7-QUOTA. Regressões puras do storage passaram 2/2; a prova real de container/rede está adicionada e aguarda CI porque o Docker Desktop local está parado. | `apps/api/src/lib/{orphanReaper.ts,orphanReaper.test.ts}`; `apps/runtime-broker/src/{orphanReaper.ts,app.test.ts}`; `packages/workspace-engine/src/index.ts` |
