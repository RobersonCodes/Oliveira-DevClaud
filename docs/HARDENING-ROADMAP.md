...
| F6-RL | ✅ **Corrigido (Fase 6, 2026-08-12; commit `af17d82`).** Limites independentes por IP confiável (`120/min`), usuário autenticado (`120/min`) e organização (`600/min`), com cadastro/login preservados em `5/min`/`8/min`. Produção compartilha os contadores no Redis; probes não são limitados; cobranças repetidas da mesma identidade na mesma requisição são deduplicadas. Regressão direcionada 6/6 e conjunto relacionado 51/51; CI Linux `31625939507` tentativa 2 aprovou imagem, lint, typecheck, testes, build e audit. A tentativa 1 parou antes das validações por HTTP 503 transitório do GitHub Releases. | `apps/api/src/{lib/rateLimits.ts,rate-limits.test.ts,app.ts,lib/auth.ts}` |
| F6-AUDIT | ✅ **Corrigido (Fase 6, 2026-08-12; commit `75bdc6c`).** Ações mutáveis já tinham cobertura ampla; foram fechadas lacunas em logout, início manual de orquestração, cancelamento de setup ainda em fila, emissão de runtime ticket e reconciliação de status terminal do agente. Metadata não contém credenciais, prompts ou logs. Regressão direcionada 29/29; CIs `31646764048`/`31646760826` e smoke `31646764020` verdes. | `apps/api/src/{audit-coverage.test.ts,integration.test.ts,runtimeGateway.test.ts,routes/auth.ts,routes/orchestrations.ts,routes/setup.ts,routes/agents.ts,lib/runtimeGateway.ts}` |
| F6-TENANT | ✅ **Corrigido (Fase 6, 2026-08-12; commit `64a0d55`).** `POST /secrets` validava o papel na organização informada, mas não confirmava que `projectId`/`workspaceId` pertenciam a ela. A rota agora rejeita combinações ambíguas, valida a cadeia de ownership antes da escrita e usa `404` indistinguível para recurso ausente/cross-tenant. Regressão de formato 9/9 e conjunto direcionado 38/38; CIs Linux/PostgreSQL `31647200113`/`31647203652` e smoke `31647200111` verdes. | `apps/api/src/{routes/secrets.ts,routes/secrets.test.ts,integration.test.ts}` |
| P1-4 | ✅ **Corrigido (Fase 6, 2026-08-12; commit `e365017`).** HTTP e WebSocket de IDE/preview compartilham o mesmo `preHandler`, que exige `DEVELOPER` antes do `101`; o Runtime Gateway também revalida membership/papel em toda requisição. A auditoria rota por rota encontrou outra assimetria: merge/rejeição direta de agente aceitava `DEVELOPER`, mas a ação equivalente de orquestração exige `ADMIN`; ambas foram alinhadas para `ADMIN`. Matriz pura com 30 células, matriz HTTP real/PostgreSQL e regressões WS aprovadas nos CIs `31628756365`/`31628759439`; smoke limpo `31628759486` verde. | `apps/api/src/{lib/runtimeProxy.ts,lib/runtimeGateway.ts,routes/agents.ts,lib/auth.ts,lib/auth.test.ts,integration.test.ts,ws-security.test.ts}`; `docs/RBAC-MATRIX.md` |
...
| P1-11 | ✅ **Corrigido (Fase 7, 2026-08-14; commit `9c7f39b`).** `destroy()` remove o bind mount somente depois do container/rede. O reaper periódico usa PostgreSQL como fonte de verdade, carência temporal, labels exatos e rechecagem por candidato; falha de banco/broker, symlink, recurso recente ou rede com endpoint desconhecido preserva o recurso para retry. A cota de disco já foi fechada em F7-QUOTA. CIs Linux/Docker `31808586839`/`31808590976` aprovaram o caso real que preserva workspace ativo e remove container/rede órfãos dentro da suíte completa (320 aprovados, 2 ignorados); smoke `31808591049` verde. | `apps/api/src/lib/{orphanReaper.ts,orphanReaper.test.ts}`; `apps/runtime-broker/src/{orphanReaper.ts,app.test.ts}`; `packages/workspace-engine/src/index.ts` |
...
| P1-14 | ✅ **Corrigido (hardening P0-4, 2026-08-10; commit `fbf6da9`, revalidado na Fase 4).** `ws-security.test.ts` cobre `Origin` ausente e maliciosa, cookie ausente, inválido e expirado, além de tentativa cross-org para terminal, IDE e preview. A suíte usa o preHandler real antes do `101`; a Fase 4 ajustou os casos de container inexistente para o Runtime Broker real no commit `5f35dfa`. É a mesma evidência de 18 testes registrada em P0-4. | `apps/api/src/ws-security.test.ts`; commits `fbf6da9` e `5f35dfa` |
| P1-15 | 🟡 **Parcialmente corrigido.** `setup-queue`, `terminal-engine`, `agent-engine` e `review-engine` já possuem `index.test.ts`: a Fase 4/commit `5f35dfa` entregou as regressões reais de terminal, agente e review contra broker/Docker, e a Fase 7/commit `d47b44b` cobriu retries do setup queue. Restam sem teste direto apenas `ide-engine` e o loop/processadores de `apps/worker/src/index.ts`. | `packages/{setup-queue,terminal-engine,agent-engine,review-engine}/src/index.test.ts`; ausência em `packages/ide-engine` e teste direto do worker confirmada pelo inventário atual |
| P1-16 | 🟡 **Parcialmente corrigido (Fase 8, 2026-08-15; commits `2637473`, `3925c51` e `e6a0645`; toolbar validada localmente, commit/CI pendentes).** Manifest/ícone/tema, worker network-only, reconexão, bottom bar com 15 destinos e layout 320px/safe areas estão implementados. A auditoria encontrou 33 controles insuficientes; após a política mobile, Chromium mediu 118 controles em 16 rotas com zero abaixo de 44×44px. A toolbar terminal acrescenta 12 teclas acessíveis de 44×44, Ctrl one-shot e foco preservado; Playwright 320px confirmou bytes de Esc/Ctrl+C/Tab/seta/símbolo, zero overflow/overlay/console. Testes direcionados 14/14, typechecks, lint e build passaram. Ainda faltam CI Docker desta mudança, demais fluxos touch, retomada automática e testes físicos de notch/teclado/rotação/conectividade. | `apps/web/app/{layout.tsx,manifest.ts,mobile-navigation.tsx,pwa-lifecycle.tsx,terminal/page.tsx,icon.png,styles.css}`; `apps/web/lib/terminalKeys.ts`; `apps/web/public/sw.js`; `apps/web/src/{pwa.test.ts,mobile-navigation.test.ts,terminal-keys.test.ts}`; `apps/web/next.config.js` |
| P1-17 | ✅ **Corrigido (Fase 7, 2026-08-12; commit `d47b44b`).** Ticks de orquestração usam 5 tentativas com backoff exponencial de 1s; provisionamento usa 3 tentativas com base de 5s; ambos têm jitter de 25%. O worker marca invariantes conhecidas como `UnrecoverableError`, sem retry, e mantém falhas transitórias retryable. Regressões puras 9/9; CIs com Redis real `31648845323`/`31648842108` e smoke `31648845333` verdes. | `packages/orchestrator-engine/src/{index.ts,index.test.ts}`; `packages/setup-queue/src/{index.ts,index.test.ts}`; `apps/worker/src/{index.ts,lib/jobErrors.ts,lib/jobErrors.test.ts}` |
| F7-CAS | ✅ **Corrigido (Fase 7, 2026-08-12; commit `5ca7566`).** Chaves internas são estáveis (`tick-<orchestrationId>` e `SetupJob.id`); merges Git gravam trailer persistente e merge direto de agente usa estado exclusivo `MERGING`. Transições críticas de orquestração/setup/agente/review usam CAS por estado e ordem uniforme de locks; repetição não duplica auditoria, cancelamento não sobrescreve conclusão e worker stale não ressuscita terminal. CIs `31659328635`/`31659325929` e smoke `31659328804` verdes. | `apps/api/src/{routes/orchestrations.ts,routes/setup.ts,routes/agents.ts,integration.test.ts}`; `apps/worker/src/index.ts`; `packages/{orchestrator-engine,setup-queue,git-engine,review-engine}/src/`; migration `20260812210000_fase7_agent_merging_state` |
| F7-HB | ✅ **Corrigido (Fase 7, 2026-08-12; commit `6bb9022`).** Heartbeat persistente/indexado para Workspace, AgentTask e Orchestration. Jobs que o worker possui renovam lease; workspaces/agents só renovam após liveness real no broker/engine. Sweep de 15s não sobrepõe, isola probes quebrados e encerra no graceful shutdown. CIs `31660067591`/`31660064853` e smoke `31660067592` verdes. | `apps/worker/src/{index.ts,lib/heartbeats.ts,lib/heartbeats.test.ts}`; `packages/database/prisma/schema.prisma`; migration `20260812213000_fase7_runtime_heartbeats` |
...
| P1-18 | 🟡 **Corrigido localmente (Fase 8, 2026-08-15; commit/CI Docker pendentes).** O handler agora consome o segundo argumento `isBinary` do evento `message`: frames textuais entregues como `Buffer` são decodificados e validados como envelopes `input`/`resize`, enquanto binários e texto legado continuam como input direto. Regressões puras cobrem JSON textual, resize fragmentado, binário/plain text e envelopes inválidos. O E2E Docker existente passou a enviar o envelope JSON real do browser, eliminando a antiga prova que só exercitava texto cru; sua execução local foi impedida por acesso negado ao daemon e deve passar no CI Linux antes de fechar o risco. | `apps/api/src/{routes/terminals.ts,lib/terminalMessages.ts,lib/terminalMessages.test.ts,e2e.test.ts}`; `apps/web/app/terminal/page.tsx` |
...
além das Fases 8-9. Dentro da Fase 3, o agendamento
