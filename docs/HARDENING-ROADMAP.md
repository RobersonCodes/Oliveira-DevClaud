...
| P1-16 | 🟡 **Parcialmente corrigido (Fase 8, 2026-08-14; commits `2637473` e `3925c51`).** Manifest/ícone/tema, worker network-only, reconexão, bottom bar com 15 destinos e layout 320px/safe areas estão implementados; CIs `31839461813`/`31839464758` e smoke `31839464854` verdes. A auditoria de touch encontrou 33 controles insuficientes em 13 rotas; após a política mobile, Chromium mediu 118 controles em 16 rotas com zero abaixo de 44×44px, sem overflow/overlay. Testes PWA/mobile 7/7, typecheck, lint e build passaram. Ainda faltam toolbar/fluxos touch, retomada automática e testes físicos de notch/teclado/rotação/conectividade. | `apps/web/app/{layout.tsx,manifest.ts,mobile-navigation.tsx,pwa-lifecycle.tsx,icon.png,styles.css}`; `apps/web/public/sw.js`; `apps/web/src/{pwa.test.ts,mobile-navigation.test.ts}`; `apps/web/next.config.js` |
