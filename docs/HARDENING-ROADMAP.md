...
| F7-REC | 🟡 **Em validação (Fase 7, 2026-08-14).** Recovery periódico usa `heartbeatAt` stale como lease e CAS para AgentTask/Orchestration. Runtime vivo retoma o tick; concluído/falho reconcilia task/run/step; ausente falha a cadeia. Orquestração sem posse ativa é reenfileirada uma única vez. Regressões puras 6/6 e typecheck/lint verdes; testes PostgreSQL de dois workers, runtime perdido e terminal concorrente aguardam CI. | `apps/worker/src/{index.ts,lib/recovery.ts,lib/recoveryStore.ts,lib/recovery.test.ts,lib/recovery.integration.test.ts}` |
