...
| P1-9 | **Parcialmente mitigado (Fase 5, atualizado em 2026-08-11).** A tag `workspace-node-v1.1.0` publicou `ghcr.io/robersoncodes/oliveira-devcloud-workspace-node` no workflow run `31445506653`, com provenance e SBOM. O índice OCI `sha256:90bacb592d8278bd7ee91f023220428663fa7087497807806e871004b2377a4a` foi confirmado remotamente, puxado por digest neste Docker host e executado como UID/GID `10001:10001`, com as versões fixadas da toolchain. `.env.production.example` e `docs/PRODUCTION-OPERATIONS.md` agora fixam produção ao digest imutável. Ainda falta repetir o pull e o fluxo operacional completo em uma VM Linux limpa; não considerar corrigido até essa evidência existir. | `infra/workspace-images/node/Dockerfile`; `.github/workflows/workspace-image.yml`; `.env.production.example`; `docs/PRODUCTION-OPERATIONS.md` |
