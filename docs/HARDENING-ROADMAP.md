...
por IP/usuário/organização passou no CI Linux. A revisão RBAC está implementada localmente e aguarda
CI com PostgreSQL; as Fases 7-9 seguem pendentes — ver Seção 7.
**Data:** 2026-08-12
...
| P1-4 | ⏳ **Correção confirmada e revisão ampliada localmente (Fase 6, 2026-08-12), aguardando CI.** HTTP e WebSocket de IDE/preview compartilham o mesmo `preHandler`, que exige `DEVELOPER` antes do `101`; o Runtime Gateway também revalida membership/papel em toda requisição. A auditoria rota por rota encontrou outra assimetria: merge/rejeição direta de agente aceitava `DEVELOPER`, mas a ação equivalente de orquestração exige `ADMIN`; ambas foram alinhadas para `ADMIN`. Matriz pura com 30 células: verde; matriz HTTP real com PostgreSQL será executada pelo CI. | `apps/api/src/{lib/runtimeProxy.ts,lib/runtimeGateway.ts,routes/agents.ts,lib/auth.ts,lib/auth.test.ts,integration.test.ts,ws-security.test.ts}`; `docs/RBAC-MATRIX.md` |
...
P1-1/P1-2/P1-3 e o rate limit por identidade foram fechados; a revisão RBAC aguarda CI.
...
avançar a Fase 6 pela validação CI da revisão RBAC HTTP/WebSocket e matriz completa de papéis,
seguida das demais etapas 6-9.
