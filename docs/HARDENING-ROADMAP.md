...
| F7-HB | ⏳ **Implementado localmente (Fase 7, 2026-08-12), aguardando CI/PostgreSQL/Docker.** Heartbeat persistente/indexado para Workspace, AgentTask e Orchestration. Jobs que o worker possui renovam lease; workspaces/agents só renovam após liveness real no broker/engine. Sweep de 15s não sobrepõe, isola probes quebrados e encerra no graceful shutdown. | `apps/worker/src/{index.ts,lib/heartbeats.ts,lib/heartbeats.test.ts}`; `packages/database/prisma/schema.prisma`; migration `20260812213000_fase7_runtime_heartbeats` |
