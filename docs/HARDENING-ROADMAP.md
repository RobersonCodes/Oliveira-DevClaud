...
| P1-17 | ⏳ **Corrigido localmente (Fase 7, 2026-08-12), aguardando CI/Redis.** Ticks de orquestração usam 5 tentativas com backoff exponencial de 1s; provisionamento usa 3 tentativas com base de 5s; ambos têm jitter de 25%. O worker marca invariantes conhecidas como `UnrecoverableError`, sem retry, e mantém falhas transitórias retryable. Regressões puras 9/9 e testes reais de recuperação/interrupção adicionados. | `packages/orchestrator-engine/src/{index.ts,index.test.ts}`; `packages/setup-queue/src/{index.ts,index.test.ts}`; `apps/worker/src/{index.ts,lib/jobErrors.ts,lib/jobErrors.test.ts}` |
