...
| P1-9 | **Parcialmente mitigado (Fase 5, 2026-08-10).** A imagem passou para `oliveira-devcloud/workspace-node:1.1.0`, foi construída e inspecionada localmente, e `.github/workflows/workspace-image.yml` prepara publicação versionada em `ghcr.io/robersoncodes/oliveira-devcloud-workspace-node` com provenance e SBOM. O caminho local e a futura imagem GHCR estão documentados em `docs/PRODUCTION-OPERATIONS.md`. Ainda não houve push/tag que publique o artefato no registry; não considerar corrigido até confirmar tag/digest remotamente e testar um pull em host limpo. | `infra/workspace-images/node/Dockerfile`; `.github/workflows/workspace-image.yml`; `docs/PRODUCTION-OPERATIONS.md` |
