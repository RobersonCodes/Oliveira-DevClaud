...
| F7-CAS | ⏳ **Corrigido localmente (Fase 7, 2026-08-12), aguardando CI/PostgreSQL/Redis/Docker.** Chaves internas são estáveis (`tick-<orchestrationId>` e `SetupJob.id`); merges Git gravam trailer persistente e merge direto de agente usa estado exclusivo `MERGING`. Transições críticas de orquestração/setup/agente/review usam CAS por estado e ordem uniforme de locks; repetição não duplica auditoria, cancelamento não sobrescreve conclusão e worker stale não ressuscita terminal. Cinco regressões reais de API e repetição real de merges adicionadas; direcionados 22/22, schema/typecheck/lint/builds verdes. | `apps/api/src/{routes/orchestrations.ts,routes/setup.ts,routes/agents.ts,integration.test.ts}`; `apps/worker/src/index.ts`; `packages/{orchestrator-engine,setup-queue,git-engine,review-engine}/src/`; migration `20260812210000_fase7_agent_merging_state` |
