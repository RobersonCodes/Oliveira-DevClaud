...
| P1-16 | 🟡 **Parcialmente corrigido (Fase 8, 2026-08-14; base `2637473`).** Manifest App Router instalável, ícone PWA/Apple, tema/viewport, worker network-only e shell de reconexão estão implementados; CIs `31830093847`/`31830096715` e smoke `31830096679` verdes. O corte mobile seguinte adicionou bottom bar persistente com todos os 15 destinos, dialog nativo para ferramentas, estado ativo e exclusão do login; safe areas nos quatro lados, espaço da home indicator e layout sem overflow a 320px. Testes direcionados PWA/mobile 6/6, typecheck, lint e build passaram; Chromium validou 15/15 rotas em `320/320`, cinco alvos persistentes 61×48, menu de onze links, login e fallback desktop. Ainda faltam auditoria dos demais touch targets, toolbar/fluxos touch, retomada automática e testes físicos de notch/teclado/rotação/conectividade. | `apps/web/app/{layout.tsx,manifest.ts,mobile-navigation.tsx,pwa-lifecycle.tsx,icon.png,styles.css}`; `apps/web/public/sw.js`; `apps/web/src/{pwa.test.ts,mobile-navigation.test.ts}`; `apps/web/next.config.js` |
