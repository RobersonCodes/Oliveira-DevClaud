...
export default function OrchestrationsPage(){
 ...
 async function load(){if(!workspaceId)return;const r=await apiFetch(`/api/v1/orchestrations?workspaceId=${workspaceId}`);if(r.ok)setFlows(await r.json())}
 ...
 async function action(id:string,path:string){setMessage('Processando...');const r=await apiFetch(`/api/v1/orchestrations/${id}/${path}`,{method:'POST'});const data=await r.json().catch(()=>({}));setMessage(r.ok?'Operação concluída.':`Falha: ${data.error??r.status}`);await load()}
 async function createDemo(){const body={workspaceId,title:'Checkout delivery',objective:'Implementar e validar checkout',startNow:true,steps:[{key:'backend',title:'Implementar backend',type:'AGENT',agent:'CODEX',prompt:'Implemente o backend necessário para concluir o checkout. Execute testes relevantes.',dependsOn:[]},{key:'frontend',title:'Revisar frontend',type:'AGENT',agent:'CLAUDE',prompt:'Revise e melhore o frontend do checkout. Preserve a arquitetura existente.',dependsOn:[]},{key:'test',title:'Executar testes',type:'SYSTEM',command:'npm test',dependsOn:['backend','frontend']},{key:'build',title:'Validar build',type:'SYSTEM',command:'npm run build',dependsOn:['test']}]};const r=await apiFetch('/api/v1/orchestrations',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(body)});setMessage(r.ok?'Orquestração iniciada.':'Falha ao iniciar.');await load()}
