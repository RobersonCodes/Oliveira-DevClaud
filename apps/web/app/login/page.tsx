...
export default function LoginPage(){
 ...
 const r=await apiFetch(`/api/v1/auth/${mode}`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(payload)});if(r.ok){setMessage('Autenticado.');location.href='/projects';}else{const j=await r.json().catch(()=>({error:'ERRO'}));setMessage(j.error??'Erro');}}
