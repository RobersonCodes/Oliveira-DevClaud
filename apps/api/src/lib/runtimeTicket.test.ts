...
describe('runtimeTicket', () => {
  ...
  it('round-trips a valid ticket', () => {
    const ticket = issueRuntimeTicket({ uid: 'user_1', workspaceId: 'ws_1', purpose: 'ide' });
  ...
  it('carries the port for preview tickets', () => {
    const ticket = issueRuntimeTicket({ uid: 'user_1', workspaceId: 'ws_1', purpose: 'preview', port: 5173 });
  ...
  it('rejects an expired ticket', () => {
    const ticket = issueRuntimeTicket({ uid: 'user_1', workspaceId: 'ws_1', purpose: 'ide' }, -1);
    expect(verifyRuntimeTicket(ticket)).toBeNull();
  });
  ...
  it('throws a clear, non-silent error when RUNTIME_TICKET_SECRET is not configured', () => {
    ...
    expect(() => issueRuntimeTicket({ uid: 'user_1', workspaceId: 'ws_1', purpose: 'ide' })).toThrow('RUNTIME_TICKET_SECRET_NOT_CONFIGURED');
  });
});
